...
class MockloadbalancerDriver (ResourceDriverInterface):
    ...
    # <editor-fold desc="Load Balancer Standard Commands">
    ...
    def get_inventory(self, context):
        ...
        '''
        resource = Mockloadbalancer.create_from_context(context)
        ...
        chassis1 = GenericChassis('Chassis 1')
        resource.add_sub_resource('1', chassis1)
        module_length = str(len(str(resource.num_modules)))
        port_length = str(len(str(resource.num_ports)))
        for module_idx in range(1, int(resource.num_modules)+1):
            module1 = GenericModule('Module ' + str.format(str(module_idx), '0'+ module_length))
            chassis1.add_sub_resource(str(module_idx), module1)
            for port_idx in range(1, int(resource.num_ports)+1):
                port1 = GenericPort('Port ' + str.format(str(port_idx), '0'+ port_length))
                module1.add_sub_resource(str(port_idx), port1)


        # TODO: Handle decreasing number of ports
        pp_length = str(len(str(resource.power_ports)))
        for pp_idx in range(1, int(resource.power_ports)+1):
            po = GenericPowerPort('Power Port ' + str(format(pp_idx, '0'+pp_length)))
            chassis1.add_sub_resource("pow{}".format(str(pp_idx)), po)

        pc_length = str(len(str(resource.port_channels)))
        for pc_idx in range(1, int(resource.port_channels)+1):
            pc = GenericPortChannel('Port Channel ' + str(format(pc_idx, '0'+pc_length)))
            resource.add_sub_resource("pc{}".format(str(pc_idx)), pc)
